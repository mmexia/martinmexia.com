...
export async function GET(req: NextRequest) {
  ...
  // If someone visits this URL directly (no OAuth params), redirect to Google Accounts
  if (!code && !state && !error) {
    return NextResponse.redirect(`${baseUrl}/botvault/google-accounts`);
  }

  if (error) {
    return NextResponse.redirect(`${baseUrl}/botvault/google-accounts?error=${encodeURIComponent(error)}`);
  }

  if (!code || !state) {
    return NextResponse.redirect(`${baseUrl}/botvault/google-accounts?error=missing_params`);
  }
  ...
  if (!storedState || storedState !== state) {
    return NextResponse.redirect(`${baseUrl}/botvault/google-accounts?error=invalid_state`);
  }
  ...
  if (!tokenRes.ok) {
    return NextResponse.redirect(`${baseUrl}/botvault/google-accounts?error=token_exchange_failed`);
  }
  ...
  if (!userInfoRes.ok) {
    return NextResponse.redirect(`${baseUrl}/botvault/google-accounts?error=userinfo_failed`);
  }
  ...
  // Redirect to Google Accounts with success indicator
  return NextResponse.redirect(`${baseUrl}/botvault/google-accounts?connected=true`);
}
